...
public class DangKyHocPhanWebController {
	...
	private void getAttribute(Model model,HttpServletRequest request, Principal principal) {
		...
		if(request.getParameter("chon")!=null) {
			...
			MonHoc monhoctruoc=monhocService.getMonHocbyID(Long.valueOf(request.getParameter("chon"))).getMonhoc();
			model.addAttribute("montienquyet",monhoctruoc);
			MonHoc monhoctienquyet=monhocService.getMonHocbyID(Long.valueOf(request.getParameter("chon"))).getMonhoctienquyet();
			model.addAttribute("monhoctienquyetdacbiet",monhoctienquyet);
			System.out.println(monhoctruoc);
		}
		...
		List<DangKyLopHocPhan> listDKHPBySVID=dangKyHPService.findAllLHPBySVID(Long.valueOf(sv.getMasv()));
		model.addAttribute("listDKHPBySVID",listDKHPBySVID);
