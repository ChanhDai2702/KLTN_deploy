...
public interface MonHocRepository  extends CrudRepository<MonHoc, Long>{
	...
	@Query(value = "select * from monhoc mh join lophocphan lhp on mh.mamonhoc=lhp.monhoc_id join giangvien gv on gv.magiangvien=lhp.giangvien_id where gv.magiangvien=:id",nativeQuery = true)
	List<MonHoc> getMHByGv(Long id);
}
